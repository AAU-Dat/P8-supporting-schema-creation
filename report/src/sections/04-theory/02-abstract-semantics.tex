...
\begin{align}
    ...
    &= (\rho_{\ab{d}}, \rho_{\ab{a}}[v_a \mapsto \mathsf{List} \left( \pi_{\mathbf{v}_t} \left(\abstable \downarrow_{\true}^{(\env{\ab{d}}, \env{\absvars})} \phi \cup \abstable \downarrow_{\unknown}^{(\env{\ab{d}}, \env{\absvars})} \phi \right) \right) \into \lookupcl(v_a) ])\\
...
\begin{align}
    \text{where } \ab{t}=\rho_{\ab{d}}(v_d)\\
    \text{and }\abstable' = \ab{t} \downarrow_{\false}^{(\env{\ab{d}}, \env{\absvars})} \phi &\cup \left\{ l\left[ \mathbf{v}_t \mapsto \pi_{\mathbf{v}_t}(s) \sqcup \pi_{\mathbf{v}_t}(l) \right] \;\middle|\; l \in \ab{t} \downarrow_{\unknown}^{(\env{\ab{d}}, \env{\absvars})} \phi \land s \in \app{\bigtimes_{i = 0}^{n}} \absexpsem{e_i}(\rho_{\ab{d}}, \rho_{\ab{a}}) \into \lookupcl(v_i)\right\} \\
    & \cup \left\{ l\left[ \mathbf{v}_t \mapsto \pi_{\mathbf{v}_t}(s) \right] \;\middle|\; l \in \ab{t} \downarrow_{\true}^{(\env{\ab{d}}, \env{\absvars})} \phi \land s \in \app{\bigtimes_{i = 0}^{n}} \absexpsem{e_i}(\rho_{\ab{d}}, \rho_{\ab{a}}) \into \lookupcl(v_i)\right\}
\end{align}
