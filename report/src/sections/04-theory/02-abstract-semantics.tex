...
\begin{align}
    ...
    \absboolsem{\exists v_n \; b}(\ab{\environment}) &= \{\true\} \cup \underset{s \in \absexpsem{v}}{\aab{\bigvee}} \absboolsem{b[s/v_n]}(\ab{\environment}) \\
    \absboolsem{\forall v_n \; b}(\ab{\environment}) &= \{\false\} \cup \underset{s \in \absexpsem{v}}{\aab{\bigwedge}} \absboolsem{b[s/v_n]}(\ab{\environment})
\end{align}
...
\begin{align}
    ...
    \begin{split}
        \absexpsem{v_t}(\rho_{\ab{d}}, \_) &=  \mathsf{List} \; \pi_{v_t}(\rho_d(v_d)) \\
...
\begin{split}
    ...
    &\mathsf{List} \Bigl( \pi_{\mathbf{v}_t} \bigl(v_d \downarrow_{\true}^{(\env{\ab{d}}, \env{\absvars})} b \cup v_d \downarrow_{\unknown}^{(\env{\ab{d}}, \env{\absvars})} b \bigr) \Bigr) \into \lookupcl(v_a) ]\biggr)
