...
To make this notion useful we assume that some mapping from application variables and table attributes to cover lattices is given by the user by some mechanism\footnote{We discuss such mechanisms in \autoref{sec:discussion}.}:\todo{Gabriela: is this assumption discussed somewhere else? does it seem like a reasonable assumption? can the intended user be expected to provide those? \\ Casper: Yes, we mention it in the discussion.}
...
\subsubsection{Abstract tables}\label{subsubsec:abstract_domain_of_tables}\todo{Gabriela: consider adding some examples}
...
\subsubsection{Abstract application variable values}\todo{Gabriela: some examples would be helpful}
...
\subsubsection{Abstract environments}\label{subsubsec:absenv}\todo{Gabriela says: some examples would be helpful}
