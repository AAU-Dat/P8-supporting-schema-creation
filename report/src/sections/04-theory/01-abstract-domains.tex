...
Now we are able to express the concretisation for regular expressions and union intervals.
\begin{align}
...
\begin{align}
    ...
    \concrete^{v_1, v_2, \dots, v_n}_5(\ab{e_1}, \ab{e_2}, \dots, \ab{e_n}) &= \concrete^{v_1}_6(\ab{e_1}) \times \concrete^{v_2}_6(\ab{e_2}) \times \dots \times \concrete^{v_n}_6(\ab{e_n})
\end{align}
...
\end{align}
With the same 'implementation' as above where $\bools = \{\true, \false, \mnull\}$.
...
Before we can define the concretisation, we define the power multiset as $\mathcal{M}(S) = \{m | m : S \rightarrow \nums\}$, and thus we define the concretisation of a table, for $attr(v_d) = (v_1, v_2, \dots, v_n)$:
\begin{align}
...
And we define the concretisation functions for environments as:
\begin{align}
...
\begin{align}
    ...
    \concrete_{3d}(\rho_{\ab{d}}) &= \left\{ \rho_{d} \in \mathfrak{E}_d \middle| \forall v_d \in \mathbb{V}_d : \rho_d(v_d) \in \concrete^{v_d}_{4d}(\rho_{\ab{d}}(v_d)) \right\}
\end{align}
\begin{align}
