...
\todo{Gabriela, are you sure about this?, wouldn't the definition given on page 145 of (Halder Cortesi), (abstract insert) be used for this and since it is based on set union, then reach a fixed point? Anders: Couldn't find anything in Halder and Cortesi paper}
...
Additionally our work is inspired by \emph{Abstract Interpretation of database queries} by Cortesi and Halder~\cite{halder_abstract_2012}.
...
Jana et al.~\cite{jana_extending_2020} have extended the application of abstract interpretation to the domain of web security. \todo{have we checked this is correct?}
...
Kashyap et al.~\cite{kashyap_integrity_2022} have proposed a new approach to integrity checking of database applications using abstract interpretation. \todo{have we checked this is correct?}
...
The current syntax lacks a way to flag such thing, but it should be possible to use the Boolean syntax fot this purpose. \todo{this needs to be claryfied}
...
Finally the we conclude the paper in \autoref{sec:conclusion}. \todo{update with examples}
