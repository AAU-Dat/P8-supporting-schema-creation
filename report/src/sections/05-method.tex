...
The database has two operations, \texttt{insert} and \texttt{update}.\todo{Casper says: there is also delete and select?}
The \texttt{insert} operation inserts a new row into the table, i.e.\ a new user, and the \texttt{update} operation updates the balance of two accounts.\todo{Casper says: insert inserts and update updates?}
...
\begin{listing}[htb!]
    ...
    \end{minted}
    \caption{SQL query for the update operation}\todo{Casper says: where does this come from?}
    \label{lst:sql-queries2}
\end{listing}


Understanding the database schema and operations is crucial as it forms the foundation for defining the behavior of the operations.\todo{Casper says: okay?}
...
\todo[inline]{Casper says: Program and graph do not match.}
...
newuser:
    ...
    \end{minted}
    \caption{Program that defines the behavior of the operations}\todo{C}
    \label{lst:program}
\end{listing}
...
This is defined by properties, which are defined by the LTL grammar on the attributes in the database.\todo{Casper says: We can still not check LTL properties.}
